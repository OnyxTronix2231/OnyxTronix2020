package robot.crossSubsystem;

public class CrossSubsystemConstants {

  public static final double STORAGE_SPEED = 0.8; //TODO check and change
  public static final double BALL_STOPPER_SPEED = 0.8; //TODO check and change
  public static final double LOADER_CONVEYOR_SPEED = 0.8; //TODO check and change
  public static final double SPIN_SHOOTER_AND_LOADER_TIME_OUT = 3; //TODO check and change
  public static final double TIME_BETWEEN_BALLS = 100; //TODO check and  change if need
  public static final double DELAY_AFTER_SHOOT = 0; //TODO check and  change if need
  public static final double LOADER_DELAY = 0.3; //TODO check and  change if need
  public static final int SHOOTER_VELOCITY = 39000; //TODO check and change

}

