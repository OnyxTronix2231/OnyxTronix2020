...
import static robot.drivetrain.DriveTrainConstants.TRAJECTORY_PARAMS.TRACKWIDTH_METERS;
...
public class OnyxTrajectoryGenerator {
  ...
  private final DifferentialDriveKinematics driveKinematics = new DifferentialDriveKinematics(TRACKWIDTH_METERS);
  ...
  private final DifferentialDriveVoltageConstraint autoVoltageConstraint =
      new DifferentialDriveVoltageConstraint(feedforward, driveKinematics, MAX_VOLTAGE);

  private final TrajectoryConfig config = new TrajectoryConfig(MAX_SPEED_METERS_PER_SECOND,
      MAX_ACCELERATION_METERS_PER_SECOND_SQUARED).setKinematics(driveKinematics).addConstraint(autoVoltageConstraint);
  ...
  public Command getAutonomousCommandFromPoses(final Pose2d startPose, final Pose2d endPose) {
    ...
    final RamseteCommand ramseteCommand =
        new RamseteCommand(trajectory, driveTrain::getPose, new RamseteController(RAMSETE_B, RAMSETE_ZETA),
            feedforward, driveKinematics, driveTrain::getWheelSpeeds, new PIDController(P_DRIVE_VEL, 0, 0),
